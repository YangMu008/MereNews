...
public class TimeUtils {
    ...
    public static String timeStrToTimelineTime(String timeStr){
        String pattern = "yyyy-MM-dd HH:mm:ss";
        return timeStrToTimelineTime(timeStr,pattern);
    }

    /**
     * 根据格式化后的时间获取时间戳
     */
    public static String timeStrToTimelineTime(String timeStr,String pattern){
        if (!AppConfig.isShowDetailTime) {
            SimpleDateFormat simpleDateFormat = new SimpleDateFormat(pattern);
            try {
                Date date = simpleDateFormat.parse(timeStr);
                long ts = date.getTime();//获取时间的时间戳
                return TimeUtils.timestampToTimelineTime(ts);
            } catch (Exception e) {
                e.printStackTrace();
                return timeStr;
            }
